...
class BlipRetrieval(BlipBase):
    ...
    def forward_image(self, image):
        ...
        cls_token_output = image_embeds[:, 0, :]
        projected_cls_token = self.vision_proj(cls_token_output)
        image_feat = F.normalize(projected_cls_token, dim=-1)
        # END TODO ###################
    ...
    def forward_text(self, caption):
        ...
        projected_cls_token = self.text_proj(cls_token_output)
        text_feat = F.normalize(projected_cls_token, dim=-1)
        # END TODO ###################
