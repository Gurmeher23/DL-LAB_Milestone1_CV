...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        image_to_text_sim = torch.matmul(image_feat, text_feat.t()) / temperature
        text_to_image_sim = torch.matmul(text_feat, image_feat.t()) / temperature

        batch_size = image.size(0)
        targets = torch.eye(batch_size, device=device, dtype=image_to_text_sim.dtype)

        loss_i2t = F.cross_entropy(image_to_text_sim, targets.argmax(dim=1))
        loss_t2i = F.cross_entropy(text_to_image_sim, targets.argmax(dim=1))
